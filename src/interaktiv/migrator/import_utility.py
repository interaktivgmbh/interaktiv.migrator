...
from interaktiv.migrator.interfaces import (
    IImportUtility,
    IConfiguration
)
...
class ImportUtility(object):
    ...
    def set_default_view(self, obj, data):
        ...
        if view_id == "view":
            return
        view_id = self.view_mapping.get(view_id, view_id)
        fti = obj.getTypeInfo()
        # set default view
    ...
    def set_portlet_blacklists(self, obj, data):
        if not data.get("portlet_blacklists", {}):
            return
        for manager_name in data["portlet_blacklists"]["value"].keys():
            manager = getUtility(
                IPortletManager,
                name=manager_name,
                context=obj
            ...
            keys = data["portlet_blacklists"]["value"][manager_name].keys()
            for key in keys:
                if data["portlet_blacklists"]["value"][manager_name][key]:
                    blacklist.setBlacklistStatus(key, True)
                else:
                    blacklist.setBlacklistStatus(key, False)
    ...
    def set_fields(self, obj, data):
        for key in data.keys():
            ...
            try:
                ...
                print "ERROR SETTING %s for %s" % (
                    key, obj.absolute_url()
                )
                pass
